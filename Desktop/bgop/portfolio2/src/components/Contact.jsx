...
const Contact = () => {
  return (
   ...
   <div className=" mb-12 lg:mb-0 lg:flex-col">
     ...
     Contactez-nous dès aujourd'hui pour discuter des besoins de votre projet et commencer à collaborer sur quelque chose d'extraordinaire !
     </p>
     <div className="flex items-center gap-2 mt-auto">
        {socialLinks.map(({ href, icon }, key )=>(
            <a key={key} href={href} className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color,color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80" target="_blank" classes="reveal-up">{icon}</a>
        ))}
     </div>

   </div>

<form action="https://getform.io/f/bvrwrrnb" method="POST" className="xl:pl-10 2xl:pl-20">
    <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
    <div className="mb-4">
    <label className="label reveal-up" htmlFor="name">
        Name
    </label>
    <input type="text" name=" name" id="name" className="text-field" autoComplete="name" required placeholder="jonh doe"/>
    ...
    <input type="email" name="email" id="email" className=" text-field" autoComplete="emain" required placeholder="jonh@exemple.com"/>
    ...
    <div className="mb-4">
    <label className="label reveal-up" htmlFor="message">
        Meassage
    </label>
    <textarea  name="message" id="massage" placeholder="Bonjour!" className="text-field resize-y min-h-32 max-h-80"></textarea>

    </div>
    <button type="submit" className="btn btn-primary [&]:max-w-full w-full justify-center reveal-up">Envoyer</button>
</form>
   </div>
